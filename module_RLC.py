...
def save_results_to_pdf(pdf_filename, T_currents, T_voltages_all, decimal_places):
    """
    ذخیره جداول نتایج در یک فایل PDF با عناوین مشخص.
    """
    with PdfPages(pdf_filename) as pdf:
        fig, ax = plt.subplots(figsize=(7, 6))
        ax.axis('off')
        ax.set_title("Current Table of Circuit", fontsize=14, pad=20)
        ...
        ax.axis('off')
        ax.set_title("Voltage Table of Circuit", fontsize=14, pad=20)
        col_labels_voltages = ["Node i", "Node j", "|V(i,j)| (V)", "Phase (deg)"]
        table_data_voltages_for_pdf = [
            [int(r[0]), int(r[1]), f"{r[2]:.{decimal_places}f}", f"{r[3]:.{decimal_places}f}"]
...
def display_node_pair_selector(T_currents, T_voltages_all, decimal_places):
    """
    نمایش ویجت کشویی و دکمه Calculate برای انتخاب زوج گره‌ها و نمایش ولتاژ و جریان.
    """
    node_pairs = [(row[0], row[1]) for row in T_voltages_all]
    dropdown = widgets.Dropdown(
        options=[f"({int(n1)}, {int(n2)})" for n1, n2 in node_pairs],
        description='Select node pair:',
        style={'description_width': 'initial'}
    )

    calculate_button = widgets.Button(
        description='Calculate',
        button_style='success',  # رنگ سبز
        tooltip='Click to calculate voltage and current',
        layout={'width': '100px'}
    )
    ...
    def on_button_clicked(b):
        with output:
            output.clear_output()
            selected_pair = eval(dropdown.value)
            n1, n2 = selected_pair
            for row in T_voltages_all:
                if int(row[0]) == n1 and int(row[1]) == n2:
                    print(f"Voltage between nodes {n1} and {n2}:")
                    print(f"|V({n1},{n2})| = {row[2]:.{decimal_places}f} V, Phase = {row[3]:.{decimal_places}f} deg")
                    break
            for row in T_currents:
                if (int(row[0]) == n1 and int(row[1]) == n2) or (int(row[0]) == n2 and int(row[1]) == n1):
                    print(f"Current between nodes {n1} and {n2}:")
                    print(f"|I({n1},{n2})| = {row[2] * 1000:.{decimal_places}f} mA")
                    break
            else:
                print(f"No direct branch between nodes {n1} and {n2}")

    calculate_button.on_click(on_button_clicked)
    display(widgets.VBox([dropdown, calculate_button]), output)